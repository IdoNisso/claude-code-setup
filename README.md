...
`statusline.sh` reads Claude Code's status JSON from stdin and renders two lines:

```
<cwd> | <branch> | +<additions> -<deletions> | <context%> (<tokens>) | <model> | <effort>
5h <pct>% (<reset>) | wk <pct>% (<reset>)
```
...
Line 2 — account usage quotas:

- **5h / wk**: 5-hour and weekly utilization, colored by level (green <60%, yellow 60–80%, red >80%), each with a countdown to its reset (`Nm` / `~Nh` / `~Nd`).
- Data comes from Anthropic's OAuth usage API, using the token in `~/.claude/.credentials.json` (read-only — the script never writes credentials).
